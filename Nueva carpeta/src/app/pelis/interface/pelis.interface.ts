// To parse this data:
//
//   import { Convert, SearchPelisResponse } from "./file";
//
//   const searchPelisResponse = Convert.toSearchPelisResponse(json);
//
// These functions will throw an error if the JSON doesn't
// match the expected interface, even if the JSON is valid.

export interface SearchPelisResponse {
    page:          number;
    results:       Peli[];
    total_pages:   number;
...
export class Convert {
    public static toSearchPelisResponse(json: string): SearchPelisResponse {
        return cast(JSON.parse(json), r("SearchPelisResponse"));
    }

    public static searchPelisResponseToJson(value: SearchPelisResponse): string {
        return JSON.stringify(uncast(value, r("SearchPelisResponse")), null, 2);
    }
}
